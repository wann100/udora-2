...
public class AdvisementEndScreen{
	...
	public AdvisementEndScreen(final UDoraTheExplorer game, final AdvisementArea area){
		this.area = area;
		this.game = game;
...
//						dispose();
	...
	public void render(){
		...
		Gdx.input.setInputProcessor(stage);
		batch.begin();
		font.draw(batch, InteractWithFiles.readAdvisementEndScreen(0), Gdx.graphics.getWidth()/2-225, Gdx.graphics.getHeight()/2+50);
		font.draw(batch, InteractWithFiles.readAdvisementEndScreen(1), Gdx.graphics.getWidth()/2-225, Gdx.graphics.getHeight()/2+25);
		batch.end();
		stage.draw();
	}
}
