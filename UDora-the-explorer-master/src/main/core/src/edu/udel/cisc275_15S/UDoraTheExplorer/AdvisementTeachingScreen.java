...
public class AdvisementTeachingScreen {
	...
	Texture buddytexture = new Texture(Gdx.files.internal("bud.jpg"));
	Actor buddyImage = new Image(buddytexture);
	
	Texture careertexture = new Texture(Gdx.files.internal("career.png"));
	TextureRegion careerregion = new TextureRegion(careertexture);
	Actor careerImage = new Image(careerregion);
	
	boolean appear1 = false;
	boolean appear2 = false;

	public AdvisementTeachingScreen(final UDoraTheExplorer game, AdvisementArea area){
		this.area = area;
		this.game = game;
	...
	public void render(){
		...
		{
			appear1 = true;
			buddyImage.setPosition(200,200);
			buddyImage.addAction(Actions.fadeIn(1f));
			stage.addActor(buddyImage);
			stage.act(Gdx.graphics.getDeltaTime());
			
		}
		else if((textLine!= 1 ) && (appear1))
		{
			buddyImage.addAction(Actions.fadeOut(1f));
			stage.addActor(buddyImage);
			stage.act(Gdx.graphics.getDeltaTime());
		}
		...
		{
			appear2 = true;
			careerImage.setPosition(250,200);
			careerImage.addAction(Actions.fadeIn(1f));
			stage.addActor(careerImage);
			stage.act(Gdx.graphics.getDeltaTime());
			
		}
		else if(((textLine!= 3 )|| (textLine!= 4 )) &&(appear2))
		{
			careerImage.addAction(Actions.fadeOut(1f));
			stage.addActor(careerImage);
			stage.act(Gdx.graphics.getDeltaTime());
		}
		...
		batch.end();
		create_dialog_box(stage,InteractWithFiles.insertNewLineSpaces(InteractWithFiles.readAdvisementTeachingScreen(textLine),40));
		stage.draw();
		if(textLine < 1){
