...
public class ResourcesTeachingScreen {
	...
	Sprite headshot = new Sprite(new Texture("tophat.png"));
	public Stage stage;
	...
	public ResourcesTeachingScreen(final UDoraTheExplorer game, ResourcesArea area){
		this.area = area;
		this.game = game;
		stage2 = new Stage();
		...
		for (int i = 0; i < InteractWithFiles.getSizeOfResourcesTeachingImages(); i++){
			screens.add(new Texture(InteractWithFiles.readResourcesTeachingImages(i)));
		}
		if (InteractWithFiles.getSizeOfResourcesTeachingImages()<InteractWithFiles.getSizeOfResourcesTeaching()){
			for (int i = InteractWithFiles.getSizeOfResourcesTeachingImages();i<InteractWithFiles.getSizeOfResourcesTeaching();i++){
				screens.add(new Texture(InteractWithFiles.readResourcesTeachingImages(InteractWithFiles.getSizeOfResourcesTeachingImages()-1)));
			}
		}
	...
	public void render(){
		...
		shape.rect(0, 0, ((textLine+1)*Gdx.graphics.getWidth()/InteractWithFiles.getSizeOfResourcesTeaching()), 50);
		shape.end();
		batch.begin();
		batch.draw(textbar, -40, -170);
		batch.draw(headshot, 52, 108);
	//	batch.begin();
		//font.draw(batch, InteractWithFiles.readResourcesTeachingScreen(1), 100, 400);
		//font.draw(batch, InteractWithFiles.readResourcesTeachingScreen(2), 100, 300);
		batch.end();
		create_dialog_box(stage,InteractWithFiles.insertNewLineSpaces(InteractWithFiles.readResourcesTeachingScreen(textLine),40));
		stage.draw();
		if(textLine < 1){
		...
		if (Gdx.input.justTouched()){
			...
			if(x > 340 && x < 470 && y > 436 && y < 472){
				if (textLine<InteractWithFiles.getSizeOfResourcesTeaching()-1){
					textLine++;
				}
				else{
					textLine = 0;
					area.screenState = ResourcesScreen.minigame;
				}
			}
		}
	}
