...
public class CalendarTeachingScreen {
	...
	Sprite headshot = new Sprite(new Texture("udeeglasses.png"));
	public Stage stage;
	...
	public CalendarTeachingScreen(final UDoraTheExplorer game, CalendarArea area){
		this.area = area;
		this.game = game;
		stage2 = new Stage();
		...
		for (int i = 0; i < InteractWithFiles.getSizeOfCalendarTeachingImages(); i++){
			screens.add(new Texture(InteractWithFiles.readCalendarTeachingImages(i)));
		}
		if (InteractWithFiles.getSizeOfCalendarTeachingImages()<InteractWithFiles.getSizeOfCalendarTeaching()){
			for (int i = InteractWithFiles.getSizeOfCalendarTeachingImages();i<InteractWithFiles.getSizeOfCalendarTeaching();i++){
				screens.add(new Texture(InteractWithFiles.readCalendarTeachingImages(InteractWithFiles.getSizeOfCalendarTeachingImages()-1)));
			}
		}
	...
	public void render(){
		...
		shape.rect(0, 0, ((textLine+1)*Gdx.graphics.getWidth()/InteractWithFiles.getSizeOfCalendarTeaching()), 50);
		shape.end();
		batch.begin();
		batch.draw(textbar, -40, -170);
		batch.draw(headshot, 52, 108);
	//	batch.begin();
		//font.draw(batch, InteractWithFiles.readCalendarTeachingScreen(1), 100, 400);
		//font.draw(batch, InteractWithFiles.readCalendarTeachingScreen(2), 100, 300);
		batch.end();
		create_dialog_box(stage,InteractWithFiles.insertNewLineSpaces(InteractWithFiles.readCalendarTeachingScreen(textLine),40));
		stage.draw();
		if(textLine < 1){
		...
		if (Gdx.input.justTouched()){
			...
			if(x > 340 && x < 470 && y > 436 && y < 472){
				if (textLine<InteractWithFiles.getSizeOfCalendarTeaching()-1){
					textLine++;
				}
				else{
					textLine = 0;
					area.screenState = CalendarScreen.minigame;
				}
			}
		}
	}
